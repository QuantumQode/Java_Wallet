...
public class SelfStorageUnit {
    ...
    public boolean add(Item e) {
        ...
        return this.storageItems.add(e);
    }
    ...
    public void keepOnlyItemsWith(int maxItemWeightInGrammes) {
        storageItems = storageItems.stream()
                .filter(item -> item.getWeightInGrammes() <= maxItemWeightInGrammes)
                .toList();
    }
    ...
    public int totalWeightInGrammes() {
        return storageItems.stream()
                .mapToInt(Item::getWeightInGrammes)
                .sum();
    }
    ...
    public double averageWeightInGrammes() {
        return storageItems.stream()
                .mapToDouble(Item::getWeightInGrammes)
                .average()
                .orElse(-1);
    }
    ...
    public SelfStorageUnit makeNewUnitWith(int maxItemWeightInGrammes) {
        Item[] filteredItems = storageItems.stream()
                .filter(item -> item.getWeightInGrammes() <= maxItemWeightInGrammes)
                .toArray(Item[]::new);
        return new SelfStorageUnit(filteredItems);
    }
    ...
    public String toString() {
        return Arrays.toString(storageItems.toArray());
    }
    ...
    public static SelfStorageUnit heaviestUnit(SelfStorageUnit[] SelfStorageUnitss) {
        if (SelfStorageUnitss.length == 0) return null;

        Comparator<SelfStorageUnit> comparator = Comparator
                .comparingInt(storage -> storage.greatestItem().getWeightInGrammes());

        return Arrays.stream(SelfStorageUnitss)
